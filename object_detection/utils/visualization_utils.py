...
def draw_side_by_side_evaluation_image(eval_dict,
    ...
    if detection_fields.detection_masks in eval_dict:
        instance_masks = tf.cast(
            tf.expand_dims(
                eval_dict[detection_fields.detection_masks], axis=0),
            tf.uint8)
    keypoints = None
    ...
    images_with_groundtruth = draw_bounding_boxes_on_image_tensors(
        ...
        tf.expand_dims(
            eval_dict[input_data_fields.groundtruth_classes], axis=0),
        tf.expand_dims(
            tf.ones_like(
...
class detected_object(ObjDict):
    __keys__ = 'location label probability'
...
def create_detection_dict(
    ...
    for i in range(min(max_boxes_to_draw, boxes.shape[0])):
        if scores is None or scores[i] > min_score_thresh:
            ...
            #             else:
            #                 class_name = 'N/A'
            #             detected_object.object_label = str(class_name)
            #     if not skip_scores:
            #         detected_object.object_prob = 100*scores[i]
            #     detection_box.objects_present[box].append(detected_object)
            #     box_to_display_str_map[box].append(object_label)
            #     print("DETECTED OBJECT: ", str(detected_object))
            #     print(str(detection_box))
            #     # print(ujson.dumps(detection_box, indent=1, sort_keys=True))
